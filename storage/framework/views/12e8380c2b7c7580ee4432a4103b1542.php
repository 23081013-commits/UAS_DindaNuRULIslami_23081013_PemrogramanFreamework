    ...
    <div class="max-w-4xl mx-auto bg-neutral-900 p-6 rounded-2xl shadow-xl border border-purple-900/40">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-purple-900/60 pb-4">
            <div class="flex gap-4">
                ...
                <a href="<?php echo e(route('servis.index')); ?>" class="text-purple-400 font-bold border-b-2 border-purple-400 pb-1 text-sm">Pendaftaran Servis</a>
                <a href="<?php echo e(route('servis.laporan')); ?>" class="text-neutral-400 hover:text-purple-400 font-medium text-sm transition">📊 Laporan Pendapatan</a>
            </div>
            <div>
        ...
        <!-- Form Input -->
        <form action="<?php echo e(route('servis.store')); ?>" method="POST" class="mb-8 bg-neutral-955/60 p-5 rounded-xl border border-purple-900/40">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="kendaraan_id" required class="mt-1 block w-full rounded-lg bg-neutral-900 border border-purple-900/60 text-white p-2.5 text-sm cursor-pointer">
                        ...
                        <?php $__currentLoopData = $kendaraans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kendaraan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($kendaraan->id); ?>" class="bg-neutral-900">
                                <?php echo e($kendaraan->no_polisi); ?> - <?php echo e($kendaraan->merk_kendaraan); ?> (<?php echo e($kendaraan->pelanggan->nama_pelanggan); ?>)
                            </option>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_servis" required value="<?php echo e(date('Y-m-d')); ?>" class="mt-1 block w-full rounded-lg bg-neutral-900 border border-purple-900/60 text-white p-2.5 text-sm scheme-dark">
                </div>
            </div>
        ...
        <div class="overflow-x-auto rounded-xl border border-neutral-800">
            <table class="min-w-full divide-y divide-neutral-800">
                ...
                <tbody class="bg-neutral-900/40 divide-y divide-neutral-800">
                    <?php $__empty_1 = true; $__currentLoopData = $serviss; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-neutral-800/30 transition">
                            <td class="px-4 py-4 text-sm text-white">
                                <span class="font-bold block text-purple-300"><?php echo e($s->kendaraan->no_polisi); ?></span>
                                <span class="text-xs text-neutral-400"><?php echo e($s->kendaraan->pelanggan->nama_pelanggan); ?></span>
                            </td>
                            <td class="px-4 py-4 text-sm text-neutral-300">
                                <div class="text-xs"><span class="text-neutral-500">Keluhan:</span> <?php echo e($s->keluhan_awal); ?></div>
                                <?php if($s->nama_sparepart): ?>
                                    <div class="text-xs mt-1 text-purple-400"><span class="text-neutral-500">Sparepart:</span> <?php echo e($s->nama_sparepart); ?> (Rp <?php echo e(number_format($s->biaya_sparepart, 0, ',', '.')); ?>)</div>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-4 text-sm font-bold text-white">
                                Rp <?php echo e(number_format($s->biaya_jasa + $s->biaya_sparepart, 0, ',', '.')); ?>

                            </td>
                            <td class="px-4 py-4 text-sm">
                                <div class="flex items-center gap-2">
                                    <!-- Form Update Status Mekanik -->
                                    <form action="<?php echo e(route('servis.updateStatus', $s->id)); ?>" method="POST" class="inline-block">
                                        <?php echo csrf_field(); ?>
                                        ...
                                        <select name="status_servis" onchange="this.form.submit()" class="text-xs font-bold rounded-lg p-1.5 border cursor-pointer transition
                                            <?php echo e($s->status_servis == 'Antri' ? 'bg-blue-950 text-blue-400 border-blue-900' : ''); ?>

                                            <?php echo e($s->status_servis == 'Dikerjakan' ? 'bg-amber-950 text-amber-400 border-amber-900' : ''); ?>

                                            <?php echo e($s->status_servis == 'Selesai' ? 'bg-green-950 text-green-400 border-green-900' : ''); ?>

                                            <?php echo e($s->status_servis == 'Dibatalkan' ? 'bg-red-950 text-red-400 border-red-900' : ''); ?>">
                                            <option value="Antri" class="bg-neutral-900" <?php echo e($s->status_servis == 'Antri' ? 'selected' : ''); ?>>Antri</option>
                                            <option value="Dikerjakan" class="bg-neutral-900" <?php echo e($s->status_servis == 'Dikerjakan' ? 'selected' : ''); ?>>Dikerjakan</option>
                                            <option value="Selesai" class="bg-neutral-900" <?php echo e($s->status_servis == 'Selesai' ? 'selected' : ''); ?>>Selesai</option>
                                            <option value="Dibatalkan" class="bg-neutral-900" <?php echo e($s->status_servis == 'Dibatalkan' ? 'selected' : ''); ?>>Dibatalkan</option>
                                        </select>
                                    </form>

                                    <?php if($s->status_servis == 'Selesai'): ?>
                                        <a href="<?php echo e(route('servis.cetakNota', $s->id)); ?>" target="_blank" class="bg-purple-700 hover:bg-purple-600 text-white text-xs px-2 py-1.5 rounded-lg transition font-bold">
                                            🖨️ Nota
                                        </a>
                                    <?php endif; ?>
                                </div>
...
</html><?php /**PATH D:\laragon\www\assignment-bengkel\resources\views/servis/index.blade.php ENDPATH**/ ?>