    ...
    <div class="max-w-4xl mx-auto bg-neutral-900 p-6 rounded-2xl shadow-xl border border-purple-900/40">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-purple-900/60 pb-4">
            <div class="flex gap-4">
                <a href="{{ route('pelanggan.index') }}" class="text-purple-400 font-bold border-b-2 border-purple-400 pb-1 text-sm">Data Pelanggan</a>
        ...
        <form action="{{ route('pelanggan.store') }}" method="POST" class="mb-8 bg-neutral-950/60 p-5 rounded-xl border border-purple-900/40">
            @csrf
                ...
                <tbody class="bg-neutral-900/40 divide-y divide-neutral-800">
                    @forelse($pelanggans as $p)
                        <tr class="hover:bg-neutral-800/30 transition">
                            <td class="px-6 py-4 text-sm text-neutral-400">{{ $p->id }}</td>
                            <td class="px-6 py-4 text-sm font-bold text-white">{{ $p->nama_pelanggan }}</td>
                            <td class="px-6 py-4 text-sm text-neutral-300">{{ $p->telepon }}</td>
                            <td class="px-6 py-4 text-sm text-neutral-400">{{ $p->alamat ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
