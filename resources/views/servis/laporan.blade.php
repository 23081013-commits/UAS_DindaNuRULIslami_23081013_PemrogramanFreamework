    ...
    <div class="max-w-4xl mx-auto bg-neutral-900 p-6 rounded-2xl shadow-xl border border-purple-900/40">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-purple-900/60 pb-4">
            <div class="flex gap-4">
                ...
                <a href="{{ route('servis.laporan') }}" class="text-purple-400 font-bold border-b-2 border-purple-400 pb-1 text-sm">📊 Laporan Pendapatan</a>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div class="p-4 bg-neutral-950 border border-neutral-800 rounded-xl">
                ...
                <span class="text-xl font-black text-blue-400">Rp {{ number_format($totalJasa, 0, ',', '.') }}</span>
            </div>
            <div class="p-4 bg-neutral-950 border border-neutral-800 rounded-xl">
                <span class="text-xs text-neutral-400 block uppercase font-bold">Total Penjualan Sparepart</span>
                <span class="text-xl font-black text-amber-400">Rp {{ number_format($totalSparepart, 0, ',', '.') }}</span>
            </div>
            <div class="p-4 bg-purple-950/40 border border-purple-500 rounded-xl">
                ...
                <span class="text-2xl font-black text-green-400">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</span>
            </div>
        </div>
                ...
                <tbody class="bg-neutral-900/40 divide-y divide-neutral-800">
                    @forelse($transaksiSelesai as $t)
                        <tr class="hover:bg-neutral-800/30 transition">
                            <td class="px-4 py-4 text-sm text-neutral-400">{{ $t->tanggal_servis }}</td>
                            <td class="px-4 py-4 text-sm text-white">
                                <span class="font-bold block">{{ $t->kendaraan->pelanggan->nama_pelanggan }}</span>
                                <span class="text-xs text-purple-400">{{ $t->kendaraan->no_polisi }}</span>
                            </td>
                            <td class="px-4 py-4 text-sm text-neutral-300">Rp {{ number_format($t->biaya_jasa, 0, ',', '.') }}</td>
                            <td class="px-4 py-4 text-sm text-neutral-300">
                                <span class="block text-xs text-neutral-400">{{ $t->nama_sparepart ?? '-' }}</span>
                                Rp {{ number_format($t->biaya_sparepart, 0, ',', '.') }}
                            </td>
                            <td class="px-4 py-4 text-sm font-bold text-green-400">Rp {{ number_format($t->biaya_jasa + $t->biaya_sparepart, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
