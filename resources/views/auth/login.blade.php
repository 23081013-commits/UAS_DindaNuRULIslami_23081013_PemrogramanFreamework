...
<body class="bg-neutral-950 min-h-screen text-gray-100 flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-neutral-900 p-8 rounded-2xl shadow-2xl border border-purple-900/40">
        ...
        @if(session('success'))
            <div class="bg-green-950/80 border border-green-500 text-green-200 px-4 py-2.5 rounded-lg text-xs mb-4">{{ session('success') }}</div>
        @endif
        @if($errors->any())
            <div class="bg-red-950/80 border border-red-500 text-red-200 px-4 py-2.5 rounded-lg text-xs mb-4">{{ $errors->first() }}</div>
        @endif
        ...
        <p class="text-center text-xs text-neutral-400 mt-4">Belum punya akun? <a href="{{ route('register') }}" class="text-purple-400 font-bold hover:underline">Daftar di sini</a></p>
    </div>
</body>
</html>